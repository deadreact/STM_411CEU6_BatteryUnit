...
public:
	...
	void onTick();
	uint16_t getFanValue() const { return m_fanValue; }
...
private:
	...
	uint16_t calcFanValue(int16_t temperature) const;
private:
	...
	uint16_t m_fanValue{0};

	CTimeout m_updateTimeout{1000};
	const std::map<int16_t, uint16_t> m_table;
};
