...
void BatteryStatus::setSOC(int val)
{
	if (val != m_soc)
	{
		...
		const int16_t width = 10 + val * 0.9f;
		capacityValue.setWidth(width);
		capacityValue.setColor(color);
		animBox.setColor(color);
		touchgfx::Unicode::snprintf(capacityTextValueBuffer, CAPACITYTEXTVALUE_SIZE, "%d", val);
...
void BatteryStatus::setVoltage(int val)
{
	...
	{
		m_voltage = val;
		touchgfx::Unicode::snprintfFloat(voltageValueBuffer, VOLTAGEVALUE_SIZE, "%.2f", val * 0.01f);
